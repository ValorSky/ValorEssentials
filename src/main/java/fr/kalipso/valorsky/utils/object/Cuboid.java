...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable{
    ...
    /**public Cuboid(Location l1, Location l2){
        if(l1.getWorld().getName().equals(l2.getWorld().getName())){
            //this.w = l1.getWorld();
            this.w_name = l1.getWorld().getName();
    ...
    public Cuboid(CustomLocation l1, CustomLocation l2){
        if(l1.getWorld().equals(l2.getWorld())){
            this.w_name = l1.getWorld().getName();
            this.xmax = (int) Math.max(l1.getX(), l2.getX());
            this.xmin = (int) Math.min(l1.getX(), l2.getX());
            this.ymax = (int) Math.max(l1.getY(), l2.getY());
            this.ymin = (int) Math.min(l1.getY(), l2.getY());
            this.zmax = (int) Math.max(l1.getZ(), l2.getZ());
            this.zmin = (int) Math.min(l1.getZ(), l2.getZ());
        }
    }
    ...
    public Iterator<Block> iterator() {
        return new CuboidIterator(new Cuboid(xmax, xmin, ymax, ymin, zmax, zmin, Bukkit.getWorld(w_name)));
    }


    public boolean hasPlayerInside(Player player){
        Location loc = player.getLocation();
        if(xmin <= loc.getX() && xmax >= loc.getX() && ymin <= loc.getY() && ymax >= loc.getY() && zmin <= loc.getZ() && zmax >= loc.getX() && w_name.equals(loc.getWorld().getName())){
            return true;
        }
        else{
            return false;
